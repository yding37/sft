...
class MultimodalDatasets(Dataset):
    def __init__(self, config, split_type='train'):
        super(MultimodalDatasets, self).__init__()

        dataset_path = config.data_path
        data = config.dataset


        dataset_path = os.path.join(dataset_path, data+'_data_noalign.pkl')
    ...
    def __getitem__(self, index):
        ...
        if self.data == 'mosi':
            META = (self.meta[index][0].decode('UTF-8'), self.meta[index]
                    [1].decode('UTF-8'), self.meta[index][2].decode('UTF-8'))
        if self.data == 'iemocap':
            Y = torch.argmax(Y, dim=-1)
        
        # pose as CE problem
        Y = torch.round(Y).long().view(-1) + 3
        return X, Y, META
