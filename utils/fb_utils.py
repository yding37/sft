"""
Some useful library files curtesy FB
https://github.com/facebookresearch/GDT/blob/873e630fa3a11d7df1415f2ccd309023af8e1bed/utils.py#L342
"""
...
def save_checkpoint(args, epoch, model, optimizer, lr_scheduler, ckpt_freq=10):
    ...
    if epoch % 10 == 0:
        torch.save(
            checkpoint,
            os.path.join(args.output_dir, 'model_weights',
                         f'model_{epoch}.pth'.format(epoch))
        )
    torch.save(
        checkpoint,
